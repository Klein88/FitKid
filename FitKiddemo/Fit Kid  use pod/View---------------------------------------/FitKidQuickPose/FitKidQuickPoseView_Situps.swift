//
//  FitKidQuickPoseView_Situps.swift
//  Fit Kid  use pod
//
//  Created by 邱文豪 on 2023/6/20.
//
...
struct FitKidQuickPoseView_Situps : View {
    ...
    @State var selectedfeatures : [QuickPose.Feature] = [.fitness(.sitUps)]
...
//            })
            if self.SetTime == 0
            {
                Text("运动结束,您仰卧起坐了\(self.jumpsecond)次 , 用时\(self.useTime)秒")
                ...
                switch status {
                    ...
                    if self.SetTime != 0{
                        if let result = features.values.first  {
                            ...
                            feedbackText = "仰卧起坐了\(counterState.count) 次"
